  ...
  <div class="part-right">
     ...
     <div class="content">
              <div class="wtcon">
               ...
               <div class="wtbox">
                 <div class="clearFix">
                     <div class="w235"><label>订  单  号：</label> <span>${order.orderNo}<input type="hidden" id="orderId" value="${order.id }"/></span> </div>
                 ...
                 <div class="clearFix">
                 <div class="wt-t">撮合金额</div>
                 <div class="clearFix">
                 	 <div class="w235"><label>应撮合金额：</label> <span>${mMatchAmount }</span> </div>
                     <div class="w235"><label>已撮合金额：</label> <span><input type="text" class="input-txt" name="matchAmount" id="matchAmount"></span> </div>
                 </div>
                 </div>
               </div>
               </div>
               <div class="btnDiv tac"><a href="javascript:history.back();" class="btnGrey" onclick="">返回</a><a href="javascript:void(0)" onclick="updateAmount()" class="btnOrage">确认</a></div>
           </div>
...
function updateAmount(){
	...
	var matchAmount = $("#matchAmount").val();
	if(matchAmount==""){
		alert("请输入撮合金额")
		return false;
	}
	if(confirm("确定输入该撮合金额")){
		$.post(
				"order/updateMatchAmount",
				{
					orderId:orderId,
				...
				function(data){
					if(data.msg == "1"){
						alert("更新成功");
						window.location.href="${pageContext.request.contextPath}/order/toPageMatchOrderAmount";
					}else{
						alert(data.msg);
					}
				},
